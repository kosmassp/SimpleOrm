...
**Level 2 is complete** (ADR-0031, 2026-09-09: exit proven by the Go port built from `spec/` + `conformance/` alone). **The programme now is uniformity across languages** (ADR-0032): the purpose is how the same spec is implemented in many language types, Fidelis is a consumer, not the target. **The PHP port is at Level 2** (ADR-0033, 2026-09-10; PHP version unchanged; the unloaded-collection guard via `unset()` + the opt-in `Navigations` trait). **Next: new ports at Levels 0–2 in one go in the order Rust → Python → TypeScript**, then the porting guide — each only when the owner says so. Do not build Level 3+ features unprompted. If I casually ask for something listed under "Deferred," tell me which level it belongs to and why it's deferred before doing anything.
...
| Feature | Level |
|---|---|
| Relationships (one-to-one, one-to-many, many-to-one, many-to-many, owned types), graph reshaping | 2 (active) — all four cardinalities declared since L2 M1 (ADR-0005/0019+add.1, composite-key FK lists); loading is M3 (default unloaded; eager on request; never on access), reshaping M4, owned types done (ADR-0030); polymorphic and "through" ruled out permanently |
| Query AST + query front-ends (fluent builder, LINQ provider) | 2 — but the criteria core (the AST: `Criteria` factories + `db.Query<T>()` with Where/OrderBy/Limit) was pulled to Level 1 by ADR-0012; the lambda front-end is per-language sugar **outside the Level 2 exit** (ADR-0029), LINQ stays out |
...
| Additional dialects — order per ADR-0023: **SQL Server first** (owner field-tests with Fidelis), **PostgreSQL second**, then MySQL/Oracle | 4 (seam exists from Level 1) — **SQL Server pulled forward by ADR-0024, PostgreSQL by ADR-0025**, runtime-first: session/CRUD/criteria/loading/SchemaGuard/migration-runner work; shadow/snapshot/diff tooling stays SQLite-only for now |
...
conformance/                  the executable definition of the library (see §9)
  ...
  amend-cases/                diff --amend scenarios: a migrations tree as data in, files deleted/written out (ADR-0017 add.3)
...
```
php/                          the PHP port, Levels 0–2 (ADR-0026, Level 2 by ADR-0033): src/ mirrors the C# areas; tests/ holds the
...
go/                           the Go port, Levels 0–2 (ADR-0027, Level 2 by ADR-0031): one module; orm/ is the public package (session,
                              registry, SchemaGuard, aliases of the internal/core contracts), orm/sqlite the dialect
                              + shadow, orm/migrations the migration model and tools, orm/cli the command as a
                              library, orm/internal/* the areas, orm/sample the fixture entities + migrations tree,
                              orm/internal/conformance one runner per folder; CODING-STANDARD.md §10 the only list
                              of Go divergences
```

Future ports live beside `dotnet/` (`rust/`) and consume `spec/` and `conformance/` unchanged.
...
await db.UpdateOnlyAsync(order, [nameof(Order.Status)], ct);   // only the listed properties (ADR-0028); same key/version rules
...
2. **Loaders produce `EntityMap`.** Three, with precedence explicit → attribute → convention:
   - Attributes: `[Table]`, `[Column]`, `[Key]`, `[Generated]`, `[Version]`, `[Ignore]`, `[EnumAsInt]`, `[Owned]` (class + navigation, ADR-0030 — a value object flattened into the owner's columns under a prefix; the `EntityMap` stays flat, property paths are dotted); declaration-only relationship metadata `[ForeignKey]`, `[ManyToOne]` (ADR-0005), `[OneToOne]`, `[OneToMany]`, `[ManyToMany]` (ADR-0019 + add.1, L2 M1; FK lists for composite keys); declaration-only DDL metadata `[Index]` (class-level, repeatable, ADR-0007 — consumed by generated DDL, snapshots, and the diff generator, ADR-0011/0017); relation sources `[View]`, `[MaterializedView]` (separate from `[View]` because it may carry `[Index]`), `[Statement]`, `[Procedure]` (ADR-0008 + addenda; every non-table source carries its defining SQL in the attribute — statements and procedures also their (name, typeof) parameter pairs; materialized views and procedures don't exist on SQLite — dormant until Level 4) — a class carries exactly one of `[Table]`/`[View]`/`[MaterializedView]`/`[Statement]`/`[Procedure]`
...
15. `Update` writes every mapped non-key column by key. **Update by column list** (ADR-0028): `UpdateOnlyAsync(entity, [property names], ct)` — a *separately named* operation (PHP `updateOnly`, Go `orm.UpdateOnly`; never an overload, the ports have no overloading) writes only the listed properties with the same key/version rules; unmapped name `CRUD-005`, key/version/generated `CRUD-006`, empty or repeated `CRUD-007`. Concurrency stays row-level. Dirty tracking stays Level 3.
...
22. Migrations are **code, per-object** (ADR-0013; owner: never external .sql files). Root versions `V0001.cs` directly under `Migrations/` compose per-object steps (`Migrations/Table/User/V0002_AddDisplayName.cs`, `View/...`) in explicit order; the root is the recorded unit. Table actions execute **rename → add → remove → raw SQL** regardless of declaration order; every action takes optional per-action `Pre`/`Post` data hooks (inline SQL). Column specs are literal (frozen); metadata-rendered DDL is legal only for an object's initial create. `SqlVersion` is the data-driven form (conformance, future generators). Malformed names `MIG-001`; duplicate versions/objects `MIG-002`; step/root version mismatch `MIG-003`; un-composed steps `MIG-004`. **Down DDL is never written — the runner derives it** (ADR-0018, superseding authoring-time generation): at `migrate down` the rollback deduces from the versioned snapshots (`V000N.schema.json` — format v2 per ADR-0017: dialect **storage types**, name-sorted columns/indexes, `key`/`generated` flags, version + generation time; written by `simpleorm snapshot` from metadata or `simpleorm shadow` from replayed history, which must agree byte-for-byte modulo `generatedAt`; **embedded as assembly resources** so rollbacks work deployed). The step's typed `RenameColumn` actions invert data-preservingly (snapshots can't tell a rename from drop+add); the rest is the snapshot diff; a NOT NULL restore lands nullable with a notice; a type/nullability change is not derivable. `Down()` is the manual override and always wins; step-level `PreDown`/`PostDown` hooks carry the data work (a data-only step derives an empty rollback). No snapshot and no override refuses `migrate down` (`MIG-020`); a failing statement reports its SQL (`MIG-021`).
23. Runner: table `schema_version(version INTEGER, object TEXT, description TEXT, checksum TEXT, applied_at TEXT, execution_ms INTEGER, primary key (version, object)) STRICT` (`applied_at` ISO-8601 UTC). Checksum = SHA-256 of the step's rendered Up SQL; drift on an applied step is `MIG-010`, history unknown to code is `MIG-011`; the whole plan validates before anything executes. The dialect provides the **run lock** — on SQLite the entire run is one `BEGIN IMMEDIATE` transaction (exclusive + atomic: a failed run applies nothing); on SQL Server one transaction holding `sp_getapplock` (ADR-0024); on Postgres one transaction holding `pg_advisory_xact_lock` with a 60s `lock_timeout` (ADR-0025 — DDL is transactional there too, so the anticipated per-migration transactions proved unnecessary). `baseline` records versions without running them. Multi-database apps use one migrations namespace (folder) per database. The diff **generator** exists (ADR-0017): `simpleorm diff` (metadata vs latest snapshot, no database; tables diff by columns, views/MVs by normalized DDL; emits literal-SQL steps with generated Downs; renames declared via `--rename`, never inferred; removals gate on `--allow-remove` `DDL-003`; type/nullability changes are `DDL-004`, hand-written; **`--amend`** regenerates the newest version in place against the snapshot history below it — hand-written versions only with `--force` (raw SQL/hooks/data aren't computable from a diff; the run names them to re-add) — ADR-0017 add.3: git is human protocol, the databases' `MIG-010` checksums are the boundary; in both modes an object with migration history but no snapshot refuses rather than being created anew), `simpleorm shadow` (rebuild snapshots by replay; `--from V000N [--to V000M]` trusts N — baseline from committed snapshots, no verification below N, regenerate only the slice), and `migrate --force [--allow-delete]` (post-migration live-schema sync: additive applies, deletions gated `DDL-003`, inexpressible reported `DDL-004`). Indexes match **structurally** (unique + ordered columns/direction), never by name (ADR-0017 add.2) — one existing under another name counts as implemented, since indexes get added directly to the database in urgencies. Snapshots cover every kind: tables by columns, view/MV/procedure by DDL (`{object, kind, asOfVersion, generatedAt, ddl}`, ADR-0017 add.1; capability-gated like their migrations). A view step's `ExpectDefinition` guard (generated steps always carry it) refuses to apply over a definition changed outside the code (`MIG-012`) — views get hotfixed in urgencies; only `--force` recreates over the drift, with notice. There is never automatic sync against shared databases.
24. The application never applies migrations at startup; it only checks (rule `MIG-030`). The CLI (`--assembly`, `--db`, `--namespace`) applies: `migrate [--force [--allow-delete]]`, `migrate down --to [--snapshots <dir>] [--force]`, `status`, `baseline`, `export-metadata`, `validate`, `snapshot --out`, `diff --out --namespace [--name] [--rename]* [--allow-remove] [--amend [--force] [--db]]`, `shadow --out [--from] [--to]`.

### Dialect seam (`IDialect`, minimal and capability-based)

25. Members at Level 1, and no more: create connection; quote identifier; parameter prefix; render `RETURNING`/generated-key retrieval; render limit/offset; render CREATE TABLE/INDEX/VIEW and the generated INSERT from `EntityMap` (ADR-0011, ADR-0008 add.3); capability flags — array parameters, transactional DDL, materialized views, procedures; migration run-lock acquire/release; describe-statement implementation; declared-type → CLR type compatibility table; storage type per mapped property (ADR-0017 — what CREATE TABLE emits, used by snapshots/diff/sync); view-definition query (ADR-0017 add.1 — backs the `MIG-012` guard and shadow view snapshots). Add members only when a second dialect needs them — SQL Server (ADR-0024) added: identifier quoting realized (`QuoteIdentifier`, identity on SQLite), `PagingRequiresOrderBy`, `SupportsRowValueIn` (false → the renderer's EXISTS rewrite), `VersionTableSql`, and the migration-action renderings (`RenameTableSql`, `RenameColumnSql`, `AddColumnSql`, `DropColumnSql`, `DropTableSql`, `DropIndexSql`) — SQLite's implementations return the pre-ADR strings byte-for-byte, so checksums and conformance pins are untouched. Postgres (ADR-0025) added `BindsTemporalsNatively` (temporals bind as UTC CLR values, not ISO strings — `timestamptz >= text` refuses; SQLite/SQL Server false). Do not speculate about Oracle.

## 7b. Level 2 milestones (ADR-0019) — one at a time; stop and report; do not start the next unprompted

1. **Relationship metadata.** *(Done, incl. add.1.)* All four classic cardinalities declared: `[OneToOne]`/`[OneToMany]` (FK on the target, by property), `[ManyToMany(typeof(Link))]` (link declared, never inferred; resolved via its `[ForeignKey]`s), beside Level 1's `[ManyToOne]`. **FK declarations are lists** — one entry per key part, in key order (composite keys, ADR-0019 add.1) — counts validated against declared key arity. Navigations: no public setter (`MAP-011`), wrong shape `MAP-020`, target FK issues `MAP-021`, link issues `MAP-022`. Declaration-only; **nothing loads implicitly — default is unloaded, eager loads with the query when requested, never on access**. Polymorphic and "through" relations are ruled out permanently. Owned types deferred until loading exists.
...
5. **Fluent front-end.** *(Moved out of Level 2 — ADR-0029.)* Typed lambda → AST stays approved as per-language sugar (ADR-0021 add.2), built on demand on an owner sample; not an exit criterion. LINQ provider stays out.
5b. **Owned types.** *(Done — ADR-0030, all three implementations.)* `[Owned]` on a class (never an entity) + `[Owned]` on the navigation; members flatten into the owner's columns under `<navigation>_` (overridable), the `EntityMap` stays **flat** (dotted property paths `Address.City`), only the mapper regroups; a nullable navigation forces member nullability and an all-NULL row reads as null; `MAP-024` for invalid declarations; no nesting, no collections, attribute loader only. PHP `#[Owned]`, Go `orm.OwnedType` marker + `orm:"owned"`. Fixture `UserProfile.Address` (sample V0010).
6. **Level 2 exit.** *(Done — ADR-0031.)* The ADR-0029 list closed: join/projection vocabulary + `conformance/ast/level2/`; export FK references by column, `MAP-017`/`MAP-011` rewordings; owned types (ADR-0030); spec sweep; benchmark refresh (ADR-0029 add.1). **Proof: the Go port extended to Level 2 with the agents forbidden to read `dotnet/` or `php/`** — 21 spec silences surfaced and answered as "Clarifications" in `spec/query-ast.md`/`spec/loading.md`, no rule changed; two reference behaviours turned into `QRY-006`. Go `orm.Load`/`LoadEach`, `Include(...).Fetch(orm.FetchMultiQuery|FetchSubSelect|FetchJoin)`; `REL-004` unreachable there — nil vs empty slice is the guard.
...
**Session state (2026-09-08, ADR-0026):** the **PHP port exists** under `php/` — Levels 0–1 complete (metadata + export, mapping, parameters, session/CRUD, criteria core, SchemaGuard, migrations runner/derived downs/snapshots/diff+amend/force-sync/shadow, CLI), built by parallel Sonnet agents against `php/CODING-STANDARD.md` and foundation contracts laid down first; every Level 0–1 conformance folder runs through a PHP runner. Its two spec findings were ruled by ADR-0029: the entity export now carries **column** names for every FK reference (`targetForeignKeyColumns`, `linkForeignKeyColumnsTo*` — the PHP/Go PascalCase workarounds are gone), and `MAP-017` reads "a name→type mapping without repeats"; `MAP-011` is stated as an intent. `amend-cases/` were neutralized (extension-less source paths, token fragments). PHP dialects for SQL Server/Postgres are not ported yet (the `Dialect` seam mirrors `IDialect` exactly). Run: `cd php && composer install && composer test`.

**Session state (2026-09-08, ADR-0027):** the **Go port exists** under `go/` — Levels 0–1 complete by the ADR-0026 method (foundation contracts + `go/CODING-STANDARD.md` first, then six parallel Sonnet agents, then a uniformity review); every Level 0–1 conformance folder runs through a Go runner (`entities` byte-identical, `ast` `sqlite` pins byte-identical, `snapshot-cases` byte-identical, `amend-cases` with `.go` appended to the extension-less paths — no case file changed). The Go shape: struct tags + an `Entity() orm.EntityDef` descriptor (typed references via generics: `orm.ForeignKey[User]("UserID")`), `*orm.Error` values, `context.Context` everywhere, `iter.Seq2` streaming, generics as package functions (`orm.Query`, `orm.Get[T]`, `orm.From[T]`), the entry types `QueryEntry`/`CommandEntry` (a type and a function cannot both be `Query`), explicit `orm.Registry` (entities, entries, migration versions, `embed.FS` snapshots) instead of assembly scanning, migration types named exactly as the reference with one Go package per object directory, the CLI as a library the application's `main` embeds, `modernc.org/sqlite` (pure Go) as the only dependency with `_txlock=immediate` mirroring Microsoft.Data.Sqlite's `BEGIN IMMEDIATE`, and SchemaGuard describing statements through the driver's column-origin API (`StatementDescriber`, an optional dialect capability beside the mirrored `Dialect` seam). `spec/errors.md` gained `MAP-023` (malformed mapping declaration — needed wherever annotations are not type-checked). Findings for an owner ruling (ADR-0027): `MAP-011` is unenforceable where fields have no setters (phrase it as an intent), `MAP-003`/`MAP-012`/`MAP-017`'s token sub-cases are unreachable in Go (typed declarations), and the `targetForeignKeyProperties` interim (PascalCase of the Go field) still stands — the column-name proposal of ADR-0026 remains open. Run: `cd go && go vet ./... && go test ./...`.

**Session state (2026-09-09, ADR-0028–0031):** `UpdateOnlyAsync` (column-list update, ADR-0028) and owned types (ADR-0030) exist in all three implementations; the ADR-0029 rulings closed the Level 2 list; the **Go port is at Level 2** (ADR-0031: renderer joins/projection/`in_select`, explicit/batch loading, all three fetch modes, `load-cases` + `ast/level2` runners — built with the agents forbidden to read the reference, which surfaced 21 spec silences now answered as "Clarifications"). **Level 2 is closed.**

**Session state (2026-09-10, ADR-0033):** the **PHP port is at Level 2** — built by the ADR-0031 method (foundation, three parallel Sonnet agents forbidden to read `dotnet/`/`go/`, a uniformity review, every gap resolved against the reference): renderer projection/joins/`in_select` with the `ast/level2` runner (7 byte-identical pins), `$db->load()`/`loadEach()`, `include()->fetch(FetchMode::MultiQuery|SubSelect|Join)` with join-mode row segmentation (`RowSegment`), the `load-cases` runner replaying every case in every mode; the ADR-0032 guard in both forms (the mapper `unset()`s collection navigations of database-read entities; the opt-in `SimpleOrm\Session\Navigations` trait makes the read `REL-004`, without it PHP's own `Error`). 15 gap questions became 13 spec Clarifications; **two reference deviations from its own spec were fixed in C#** (join-mode refusal precedence; one-to-many collections sorted value-wise after fetching — a decimal key loaded in TEXT order) and alias uniqueness became `QRY-006` in both. PHP: 418 tests. Next per ADR-0032: **Rust** at Levels 0–2, only when the owner says so.
...
- `diff-cases/`: generator scenarios — current shape + latest snapshot + declared renames in, the exact change out (no database). `snapshot-cases/`: a fixture entity + version + pinned time in, the exact exported snapshot document out. `amend-cases/` (ADR-0017 add.3): a migrations tree as data (files with/without the generator's header, snapshots at versions) + the `diff` command in, the exit code and the files deleted/written/kept out — against a fixed fixture each implementation defines natively.
...
- **Port order (ADR-0023, extended by ADR-0032): PHP first** (done to Level 2 — ADR-0026/0033), **Go** (done to Level 2 — ADR-0027/0031), then **Rust → Python → TypeScript**, each at Levels 0–2 in one go — the spec-neutrality stress tests (traits/derive macros, dataclasses, decorators instead of attributes; no exceptions, a synchronous standard driver, no static types, ownership of loaded graphs): their purpose is to find places where the spec is secretly shaped by one language family, and the port changing the spec is the point. Port agents may not read another implementation (ADR-0031's rule); every unanswered question goes into the port's gap list, which becomes spec "Clarifications". A porting guide follows the sixth implementation.
