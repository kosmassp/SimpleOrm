...
go/                           the Go port, Levels 0–1 (ADR-0027): one module; orm/ is the public package (session,
...
6. **Level 2 exit.** *(In progress — ADR-0029 sets the list.)* Done: join/projection vocabulary in `spec/query-ast.md` + `conformance/ast/level2/` (7 cases); export FK references by **column** (`targetForeignKeyColumns`), `MAP-017` rephrased, `MAP-011` an intent; owned types (ADR-0030). Spec sweep done; benchmark refresh done (ADR-0029 add.1: criteria within 10% of inline SQL; SubSelect is the fastest fetch mode on SQLite, Join allocates 2×). **Proof: the Go port extended to Level 2 from `spec/` + `conformance/` alone.** Remaining: Go L2 port → declare exit.
