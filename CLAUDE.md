...
6. **Level 2 exit.** *(In progress — ADR-0029 sets the list.)* Done: join/projection vocabulary in `spec/query-ast.md` + `conformance/ast/level2/` (7 cases); export FK references by **column** (`targetForeignKeyColumns`), `MAP-017` rephrased, `MAP-011` an intent; owned types (ADR-0030). Owed: a spec sweep for overtaken "future" wording; benchmark refresh (criteria + loading). **Proof: the Go port extended to Level 2 from `spec/` + `conformance/` alone.** Remaining order: benchmarks → Go L2 port → declare exit.
