...
**Level 2 is complete** (ADR-0031, 2026-09-09: exit proven by the Go port built from `spec/` + `conformance/` alone). **The next target is not chosen yet** — Level 3 (session semantics), the PHP Level 2 upgrade for Fidelis, the Rust port, or Level 4 dialect tooling are the candidates; ask the owner before starting any of them. Do not build Level 3+ features unprompted. If I casually ask for something listed under "Deferred," tell me which level it belongs to and why it's deferred before doing anything.
