...
**Level 2 is complete** (ADR-0031, 2026-09-09: exit proven by the Go port built from `spec/` + `conformance/` alone). **The programme now is uniformity across languages** (ADR-0032): the purpose is how the same spec is implemented in many language types, Fidelis is a consumer, not the target. **Next: the PHP Level 2 upgrade** (PHP version unchanged; the unloaded-collection guard via `unset()` + the opt-in `Navigations` trait), then new ports at Levels 0–2 in one go in the order **Rust → Python → TypeScript**, then the porting guide. Do not build Level 3+ features unprompted. If I casually ask for something listed under "Deferred," tell me which level it belongs to and why it's deferred before doing anything.
