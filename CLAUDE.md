...
5. SQL lives in `.sql` files as embedded resources under `Sql/`, referenced by relative path. `Query.Inline(...)` is the explicit escape hatch; the validator checks it too. Exception (ADR-0008 addendum 2): a `[Statement]` entity carries its SQL inline in the attribute together with its declared parameter contract — the class is fully self-contained.
