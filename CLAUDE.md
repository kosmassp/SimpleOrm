...
3. **Explicit + batch loading.** *(Done, ADR-0021.)* `LoadAsync(entity, nameof(T.Nav), ct)` + `LoadEachAsync(entities, …)` — one criteria query per navigation per call (many-to-many: two — link then targets), chunked at 500 owners, ordered by target key, composite keys via OR-of-ANDed tuples; owners sharing a many-to-one target share the instance. Nothing loads implicitly; reading an unloaded navigation never fires SQL (no proxies). `REL-001` unknown navigation, `REL-002` one-to-one matched >1 row, `REL-003` load-time shape disagreement. `spec/loading.md` + `conformance/load-cases/`. Open owner question: should unloaded access *throw* instead of reading empty/null?
4. **Eager loading.** *(Done, ADR-0022 — amended from the join sketch.)* `Include(navigations…)` on the criteria chain: root query + one batch load per navigation (multi-query — Eloquent's strategy: correct paging, visible round trips, per-result identity via M3's shared instances). `REL-001` even on empty results. Joins in `SelectAst` + joined-row reshaping moved out of L2 exit — they land when filtering-on-related arrives; JSON nesting stays the single-round-trip option. **Open owner question**: is a join-based single-query include mode still wanted?
