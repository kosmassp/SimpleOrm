...
5b. **Owned types.** *(Pending — pulled into Level 2 by ADR-0029.)* A value object stored as columns of its owner's table: metadata kind, mapping rules, export-format change, spec + conformance; both ports follow.
6. **Level 2 exit.** *(Pending — ADR-0029 sets the list.)* Owed: join/projection vocabulary in `spec/query-ast.md` + join cases in `conformance/ast/`; export FK references by **column** (`targetForeignKeyColumns`), `MAP-017` rephrased as "name→type mapping without repeats", `MAP-011` stated as an intent; a spec sweep for overtaken "future" wording; benchmark refresh (criteria + loading). **Proof: the Go port extended to Level 2 from `spec/` + `conformance/` alone.** Proposed order: joins spec → export/wording fixes → owned types → benchmarks → Go L2 port → declare exit.
