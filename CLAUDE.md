...
24. The application never applies migrations at startup; it only checks (rule `MIG-030`). The CLI (`--assembly`, `--db`, `--namespace`) applies: `migrate [--force [--allow-delete]]`, `migrate down --to`, `status`, `baseline`, `export-metadata`, `validate`, `snapshot --out`, `diff --out --namespace [--name] [--rename]* [--allow-remove]`, `shadow --out [--from] [--to]`.
