...
| `db.LoadAsync(entity, nameof(T.Nav), ct)` / `LoadEachAsync(list, …)` (Level 2 loading, spec/loading.md) | `orm.Load(ctx, db, &entity, "Nav")` / `orm.LoadEach(ctx, db, []*T, "Nav")` — pointers, so loaded state lands on the caller's instances; `Repository[T].Load/LoadEach` forward | generic methods are package functions (above) |
| `Include(navigations…).Fetch(FetchMode.SubSelect)` on the criteria chain | `orm.From[T](db).Include("Nav", …).Fetch(orm.FetchSubSelect)`; modes `orm.FetchMultiQuery` (default), `orm.FetchSubSelect`, `orm.FetchJoin`; the single-row terminals eager-load identically | same chain, same names |
| `REL-004` — an unloaded collection navigation of a database-read entity throws on access | **unreachable**: a slice read cannot be intercepted. The Go expression of the guard is the nil/empty distinction — a never-loaded collection is a **nil** slice, a loaded-but-empty one is an **empty non-nil** slice (`[]*T{}`); a singular navigation is a nil pointer until loaded, and after loading nil means a null FK or a dead link | no proxies, no interception (spec/loading.md: "where the language cannot intercept a property read") |
| `SelectAst` joins/projection and `Criteria.InSelect` (Level 2 AST, spec/query-ast.md) | `core.SelectAst.Projection/Joins`, `core.SelectJoin{Target, Alias, ParentAlias, On []JoinPair, Project}`, `core.InSelect(properties, subquery)`; built by eager loading, replayed by the `ast/level2` runner, never exposed by the chain | same tree, Go field names |
