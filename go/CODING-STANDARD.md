...
| `[Statement("sql", "since", typeof(DateTime))]` pair stream (`MAP-017` odd count / wrong token) | `orm.Statement(sql, orm.Param[time.Time]("since"))` — typed pairs; only the duplicate-name sub-case is reachable | the finding ADR-0026 raised: the spec should phrase MAP-017 as "a name→type mapping without repeats" |
...
| `MAP-011` (a navigation exposes a public setter) | not enforced: Go fields have no setters and a navigation must be exported for the library to populate it; navigations are library-written by convention (documented on each) | unenforceable in Go; flagged for the spec as an intent rather than a rule |
...
| `targetForeignKeyProperties` / `linkForeignKeysTo*` exported as C# property names (`UserId`) | exported as `core.ToPascalCase(field)` — `UserID` → `UserId` — so the export is byte-identical | interim: the spec should switch these fields to column names — flagged in ADR-0026 and ADR-0027 |
